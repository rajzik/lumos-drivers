...
To contribute to Lumos-drivers, begin by cloning the repository, installing dependencies, and
building initial files.

```bash
git clone git@github.com:rajzik/lumos-drivers.git && cd ./lumos-drivers
yarn install
yarn run build
```

Lumos-drivers uses [Yarn workspaces](https://yarnpkg.com/lang/en/docs/workspaces/) to manage
multiple packages as a monorepo, and [Beemo](https://github.com/beemojs/beemo) as the configuration
...
Please report all bugs as [an issue](https://github.com/rajzik/lumos-drivers/issues/new) on the
Lumos-drivers repository. Before filing a new issue, try to make sure your problem doesn’t already
exist.
...
1. Dependencies should be updated in isolation per package (workspace), and commited to git
   separately. For example, only dependencies within the `@rajzik/driver-jest` package should be
...
3. Commit all dependency updates in the same branch, so they can be grouped under the same PR. Once
   the PR has been approved, squash or rebase merge (_not a normal merge_) the PR so all commits and
   their formatted messages are sent to master. This will trigger an auto-release with the correct
   version bumps.

<!-- prettier-ignore -->
[conventional-changelog-beemo]: https://github.com/rajzik/conventional-changelog-beemo
